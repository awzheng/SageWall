...
To start using the app, you must wake up the "Brain" on AWS.
* Start the notebook file `SageWall_Training.ipynb` in your editor. 
* **Wait:** This process takes about 3 to 5 minutes. It is provisioning a dedicated server on AWS.
* Select the final cell labeled `Delete Endpoint`, then select `Run` -> `Run All Above Selected Cell`.
* **Wait Again:** The code takes up to 10 minutes to train and deploy the model.
...
* If the name is incorrect, the app will show a connection error.
* Unable to locate credentials: You need to provide your AWS Access Keys to your local environment so `boto3` can connect.
...
* **Action:** If the score is high, the system simulates blocking the IP. If low, it simulates allowing the traffic to pass.
